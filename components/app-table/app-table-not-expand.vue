<template>
	<view class="cmp_table_not_expand_container"> 
		<view class="cmp_table_wrap">
			<view class="cmp_table_header">
				<view class="cmp_table_header_group">
					<view class="cmp_table_header_td color_blue_btb" 
						  v-for="(item, index) in headerData" 
						  :style="{ 'text-align':item.center ? 'center' :'left' }" 
						  :key='index'>{{item.text}}</view>
				</view>
			</view> 
			<view class="cmp_table_body">
				<view class="cmp_table_body_tr" v-for="(item, index) in bodyData" :key="index">
		...
		<view class="not_data" v-if="bodyData && bodyData.length == 0">
			暂无数据
		</view>
