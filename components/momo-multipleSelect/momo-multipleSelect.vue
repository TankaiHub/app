		...
		<view class="select-box" :class="activeClass ? 'select-box-show' : ''">
			...
			<view class="body-warp">
				<view class="body">
					...
					<view v-else>
						<view class="select-item"
							 :class="[selectedArr[index] ? 'selected' : '']" 
							 v-for="(item,index) in data" 
							 :key="item+index" 
							 @tap="onPluralSelect(item, index)">
		...
		methods: {
				...
				if (this.data.length && this.defaultSelected.length) {
					this.data.forEach((item, i) => {
						for (let n = 0; n < this.defaultSelected.length; n++) {
							if (this.isPlural) {
								if (!item.disabled && item[this.valueName] === this.defaultSelected[n]) {
									this.selectedArr.splice(i, 1, true);
									break;
								}
							}else { 
								if (item == this.defaultSelected[n]) {
									this.selectedArr.splice(i, 1, true);
								}
							}
						}
			...
			onPluralSelect(item, index) {
				if (item == '其他') {
					...
					if(item == '无') {
						this.selectedArr = [];
						this.selectedArr = this.data.map(el => false);
						this.selectedArr[len - 1] = true;
					}else { 
						this.selectedArr[len - 1] = false;
					}
			...
			onAllToggle() {
				...
				var len = this.selectedArr.length;
